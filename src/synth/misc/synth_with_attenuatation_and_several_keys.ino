...
void updateControl(){
    ...
    if (!digitalRead(BTN_1)) {
        freq_to_set[0] = Octaves[index_Octave][0];
        ampl_to_set[0] = 1;
        Serial.println("on btn1");
    }
    if (digitalRead(BTN_1)) {           // Уменьшение значение амплитуды, если клавиша на нажата
        ampl_to_set[0] /= 1.00005;      // за 1 сек в 2 раза, за 2 сек в 5 раз, за 3 сек в 12 раз (можно сказать 0)
    }


    if (!digitalRead(BTN_2)) {
        freq_to_set[1] = Octaves[index_Octave][1];
        ampl_to_set[1] = 1;
        Serial.println("on btn2");
    }
    if (digitalRead(BTN_2)) {
        ampl_to_set[1] /= 1.00005;
    }

    if (!digitalRead(BTN_3)) {
        freq_to_set[2] = Octaves[index_Octave][2];
        ampl_to_set[2] = 1;
        Serial.println("on btn3");
    }
    if (digitalRead(BTN_3)) {
        ampl_to_set[2] /= 1.00005;
    }

    if (!digitalRead(BTN_4)) {
        freq_to_set[3] = Octaves[index_Octave][3];
        ampl_to_set[3] = 1;
        Serial.println("on btn4");
    }
    if (digitalRead(BTN_4)) {
        ampl_to_set[3] /= 1.00005;
    }

    if (!digitalRead(BTN_5)) {
        freq_to_set[4] = Octaves[index_Octave][4];
        ampl_to_set[4] = 1;
        Serial.println("on btn5");
    }
    if (digitalRead(BTN_5)) {
        ampl_to_set[4] /= 1.00005;
    }

    if (!digitalRead(BTN_6)) {
        freq_to_set[5] = Octaves[index_Octave][5];
        ampl_to_set[5] = 1;
        Serial.println("on btn6");
    }
    if (digitalRead(BTN_6)) {
        ampl_to_set[5] /= 1.00005;
    }

    if (!digitalRead(BTN_7)) {
        freq_to_set[6] = Octaves[index_Octave][6];
        ampl_to_set[6] = 1;
        Serial.println("on btn7");
    }
    if (digitalRead(BTN_7)) {
        ampl_to_set[6] /= 1.00005;
    }

    if (!digitalRead(BTN_8)) {
        freq_to_set[7] = Octaves[index_Octave][7];
        ampl_to_set[7] = 1;
        Serial.println("on btn8");
    }
    if (digitalRead(BTN_8)) {
        ampl_to_set[7] /= 1.00005;
    }

    if (!digitalRead(BTN_9)) {
        freq_to_set[8] = Octaves[index_Octave][8];
        ampl_to_set[8] = 1;
        Serial.println("on btn9");
    }
    if (digitalRead(BTN_9)) {
        ampl_to_set[8] /= 1.00005;
    }

    if (!digitalRead(BTN_10)) {
        freq_to_set[9] = Octaves[index_Octave][9];
        ampl_to_set[9] = 1;
        Serial.println("on btn10");
    }
    if (digitalRead(BTN_10)) {
        ampl_to_set[9] /= 1.00005;
    }

    if (!digitalRead(BTN_11)) {
        freq_to_set[10] = Octaves[index_Octave][10];
        ampl_to_set[10] = 1;
        Serial.println("on btn11");
    }
    if (digitalRead(BTN_11)) {
        ampl_to_set[10] /= 1.00005;
    }

    if (!digitalRead(BTN_12)) {
        freq_to_set[11] = Octaves[index_Octave][11];
        ampl_to_set[11] = 1;
        Serial.println("on btn12");
    }
    if (digitalRead(BTN_12)) {
        ampl_to_set[11] /= 1.00005;
    }
...
AudioOutput_t updateAudio(){
    ...
    if (number_of_active_keys != 0) {
        All_signal = (ampl_to_set[0] * aSin1.next() + ampl_to_set[1] * aSin2.next() + ampl_to_set[2] * aSin3.next() +
                      ampl_to_set[3] * aSin4.next() + ampl_to_set[4] * aSin5.next() + ampl_to_set[5] * aSin6.next() +
                      ampl_to_set[6] * aSin7.next() + ampl_to_set[7] * aSin8.next() + ampl_to_set[8] * aSin9.next() +
                      ampl_to_set[8] * aSin10.next() + ampl_to_set[10] * aSin11.next() +
                      ampl_to_set[11] * aSin12.next()) /
                     number_of_active_keys;
    }
    else{All_signal /= 1.0001;}                 // Берём сигнал, посчитанный ранее, и делим чтобы он угасал.
    return MonoOutput::from8Bit(int(All_signal)); // return an int signal centred around 0
}
